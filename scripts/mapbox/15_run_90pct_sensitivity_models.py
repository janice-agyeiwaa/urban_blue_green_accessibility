...
# ============================================================
# 15_run_90pct_sensitivity_models.py
#
# Purpose:
# Run the four accessibility mixed-effects models using only
# parks with >=90% temporal coverage.
#
# Same specification as the 103-site main models:
# - BFGS optimizer
...
#
# Only difference:
# sample restricted to coverage_pct >= 90
# ============================================================
...
# ------------------------------------------------------------
# Restrict to >=90% temporal coverage
# ------------------------------------------------------------

if "coverage_pct" not in df.columns:
    raise ValueError(
        "coverage_pct not found in dataset."
    )
...
# ------------------------------------------------------------
# Standardize continuous Level-2 variables
#
# IMPORTANT:
# Recalculate mean and SD using the 78 parks in THIS
# sensitivity sample.
# ------------------------------------------------------------
...
print("\nSensitivity-sample standardization:")
...
# ------------------------------------------------------------
# Join standardized variables back
# ------------------------------------------------------------
...
def fit_access_model(model_name, predictor):
    ...
    print("\n" + "=" * 80)
    print(f"{model_name.upper()} ACCESSIBILITY MODEL - >=90% COVERAGE")
    print("=" * 80)
    ...
    coef_file = (
        OUTPUT_DIR
        / f"{model_name}_90pct_coefficients.csv"
    )
    ...
    summary_file = (
        OUTPUT_DIR
        / f"{model_name}_90pct_summary.txt"
    )
...
print("\n" + "=" * 100)
print(">=90% COVERAGE SENSITIVITY MODEL COMPARISON")
print("=" * 100)
